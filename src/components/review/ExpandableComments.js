...
const Expandable = ({ children, reviewId }) => {
    ...
    const { getComments, addComment, isOpen, toggleOpen, loading } = ReviewContext;
    ...
    return (
        <div>
            <Button variant='outlined' color='primary' style={{ color: 'rgb(26, 33, 46)', borderColor: 'rgb(26, 33, 46)', border: '2px solid', 'marginBottom': '20px' }} onClick={onClick}>{isOpen ? 'Close' : 'Open'}</Button>
            {isOpen && children}
            {isOpen && <div >
                <TextareaAutosize
